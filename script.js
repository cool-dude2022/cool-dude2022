...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const yourLevelInput = document.getElementById('yourLevel');
  const victimLevelInput = document.getElementById('victimLevel');
  ...
  const gemModeKill = document.getElementById('gemModeKill');
  ...
  const gemKillSection = document.getElementById('gemKillSection');
  ...
  // Handle gem mode (buy vs kill vs sell) toggle
  gemModeBuyXp.addEventListener('change', () => {
    if (gemModeBuyXp.checked) {
      showElement(gemBuyXpSection);
      hideElement(gemKillSection);
      hideElement(gemSellSection);
    }
  });
  
  gemModeKill.addEventListener('change', () => {
    if (gemModeKill.checked) {
      hideElement(gemBuyXpSection);
      showElement(gemKillSection);
      hideElement(gemSellSection);
    }
  });
  ...
  xpForm.addEventListener('submit', (ev) => {
    ...
    if (modeTo.checked) {
      ...
      if (gemModeSell.checked) {
      ...
      } else if (gemModeKill.checked) {
        // Kill calculator: calculate XP gained and new level after killing someone
        const yourCurrentLevel = Math.max(1, Math.floor(parseNumberInput(yourLevelInput.value) || 30));
        const victimCurrentLevel = Math.max(1, Math.floor(parseNumberInput(victimLevelInput.value) || 50));
        
        // Calculate victim's total XP (from level 1 to their level)
        const victimTotalXp = closedForm(1, victimCurrentLevel);
        
        // Apply kill tax if enabled
        ...
        if (useTax) {
          ...
          summaryText += ` → You gain ${formatNumberWithCommas(xpGained)} XP`;
        }
