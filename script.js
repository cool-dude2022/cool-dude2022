...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const levelsNormal = document.getElementById('levelsNormal');
  const levelsKill = document.getElementById('levelsKill');
  const levelsNormalSection = document.getElementById('levelsNormalSection');
  const levelsKillSection = document.getElementById('levelsKillSection');
  const yourLevelByLevelsInput = document.getElementById('yourLevelByLevels');
  const victimLevelByLevelsInput = document.getElementById('victimLevelByLevels');
  ...
  // Handle levels mode (normal vs kill) toggle
  levelsNormal.addEventListener('change', () => {
    if (levelsNormal.checked) {
      showElement(levelsNormalSection);
      hideElement(levelsKillSection);
    }
  });
  
  levelsKill.addEventListener('change', () => {
    if (levelsKill.checked) {
      hideElement(levelsNormalSection);
      showElement(levelsKillSection);
    }
  });
  ...
  xpForm.addEventListener('submit', (ev) => {
    ...
    if (modeTo.checked) {
      ...
      if (levelsKill.checked) {
        // Kill calculator in By levels mode
        const yourCurrentLevel = Math.max(1, Math.floor(parseNumberInput(yourLevelByLevelsInput.value) || 30));
        const victimCurrentLevel = Math.max(1, Math.floor(parseNumberInput(victimLevelByLevelsInput.value) || 50));
        
        // Calculate victim's total XP (from level 1 to their level)
        const victimTotalXp = closedForm(1, victimCurrentLevel);
        
        // Apply kill tax if enabled in gems section
        const { taxRate, useTax } = getGemSettings();
        let xpGained = victimTotalXp;
        if (useTax) {
          xpGained = victimTotalXp * (1 - taxRate);
        }
        ...
        summaryText += `Killing level ${victimCurrentLevel} player (${formatNumberWithCommas(victimTotalXp)} XP)`;
        
        if (useTax) {
          summaryText += ` → After ${(taxRate * 100).toFixed(1)}% kill tax, you gain ${formatNumberWithCommas(xpGained)} XP`;
        } else {
          summaryText += ` → You gain ${formatNumberWithCommas(xpGained)} XP (no tax)`;
        }
        ...
        // Normal mode - gain N levels
        const n = Math.max(1, Math.floor(parseNumberInput(levelsUpInput.value) || 1));
        target = current + n;
        usedXp = closedForm(current, target);
        
        summary.textContent = `Total XP required to go from level ${current} to level ${target}: ${formatNumberWithCommas(usedXp)} XP (closed-form)`;
        hideElement(progressSection);
      }
    } else if (modeXp.checked) {
      ...
      if (gemModeSell.checked) {
        ...
        let summaryText = `With ${formatNumberWithCommas(gemsInput)} gems`;
        if (useTax) {
          const taxAmount = gemsInput * taxRate;
          const netGems = gemsInput * (1 - taxRate);
          summaryText += ` (tax: ${formatNumberWithCommas(taxAmount)}, net: ${formatNumberWithCommas(netGems)})`;
        }
        summaryText += `, you can buy ${formatNumberWithCommas(xpFromGems)} XP and reach level ${Math.floor(fractionalLevel)}${progressText}. (Exact fractional level: ${fractionalLevel.toFixed(4)})`;
