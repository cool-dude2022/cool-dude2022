...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const gemModeBuy = document.getElementById('gemModeBuy');
  const gemModeSell = document.getElementById('gemModeSell');
  const gemBuySection = document.getElementById('gemBuySection');
  ...
  function getGemSettings() {
    ...
    return { costPerMillion, taxRate };
  }

  function gemsToXp(gems) {
    const { costPerMillion, taxRate } = getGemSettings();
    // After tax deduction, you get (1 - taxRate) of the XP
    const afterTax = gems * (1 - taxRate);
  ...
  gemModeBuy.addEventListener('change', () => {
    if (gemModeBuy.checked) {
      showElement(gemBuySection);
  ...
  xpForm.addEventListener('submit', (ev) => {
    ...
    if (modeTo.checked) {
      ...
      // Gems mode
      if (gemModeSell.checked) {
        // Sell mode: calculate gem value of selling X levels
        ...
        // Calculate XP from current level down by levelsToSell
        const newLevel = current - levelsToSell;
        target = newLevel;
        usedXp = closedForm(newLevel, current);
        const gemsValue = xpToGems(usedXp);
        const { taxRate, costPerMillion } = getGemSettings();
        const taxAmount = gemsValue * taxRate;
        const netGems = gemsValue * (1 - taxRate);
        
        summary.textContent = `Selling ${levelsToSell} level${levelsToSell > 1 ? 's' : ''} (from level ${current} to level ${newLevel}) = ${Math.round(usedXp).toLocaleString()} XP worth ${Math.round(gemsValue).toLocaleString()} gems (tax: ${Math.round(taxAmount).toLocaleString()}, net: ${Math.round(netGems).toLocaleString()}).`;
        
        note.textContent = `After selling, you'll be level ${newLevel}. Rate: ${costPerMillion.toLocaleString()} gems per 1M XP with ${(taxRate * 100).toFixed(1)}% tax.`;
        showElement(note);
        hideElement(progressSection);
        
      } else {
        // Buy mode: calculate level from gems
        ...
        summary.textContent = `With ${gemsInput.toLocaleString()} gems (tax: ${taxAmount.toLocaleString()}, net: ${netGems.toLocaleString()}), you can buy ${Math.round(xpFromGems).toLocaleString()} XP and reach level ${Math.floor(fractionalLevel)}${progressText}. (Exact fractional level: ${fractionalLevel.toFixed(4)})`;
        
        if (leftover >= 0) {
          const leftoverGems = (leftover / 1000000) * costPerMillion * (1 - taxRate);
          note.textContent = `XP consumed to reach level ${target}: ${Math.round(Math.max(0, usedXp)).toLocaleString()} XP. Leftover: ${Math.round(leftover).toLocaleString()} XP (worth ~${leftoverGems.toLocaleString()} gems after tax).`;
          showElement(note);
        } else {
          note.textContent = '';
          hideElement(note);
