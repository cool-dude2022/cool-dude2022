  ...
  xpForm.addEventListener('submit', (ev) => {
    ...
    if (modeTo.checked) {
      ...
      if (reverseCalcCheckbox.checked) {
        ...
        target = targetLevel;
        usedXp = closedForm(current, target);
        const gemsNeeded = xpToGems(usedXp);
        const { taxRate } = getGemSettings();
        const taxAmount = gemsNeeded * taxRate;
        const netGems = gemsNeeded * (1 - taxRate);
        
        summary.textContent = `To go from level ${current} to level ${target}, you need ${Math.round(gemsNeeded).toLocaleString()} gems (tax: ${Math.round(taxAmount).toLocaleString()}, net: ${Math.round(netGems).toLocaleString()}). This buys ${Math.round(usedXp).toLocaleString()} XP.`;
        
        const { costPerMillion } = getGemSettings();
        note.textContent = `Rate: ${costPerMillion.toLocaleString()} gems per 1M XP with ${(taxRate * 100).toFixed(1)}% tax.`;
        showElement(note);
        hideElement(progressSection);
        
      } else {
        // Normal mode: calculate level from gems
