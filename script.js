...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Handle tax toggle - show/hide tax settings
  applyTaxCheckbox.addEventListener('change', () => {
    if (applyTaxCheckbox.checked) {
      showElement(taxSettings);
    } else {
      hideElement(taxSettings);
    }
  });
  ...
  gemModeBuyXp.addEventListener('change', () => {
    if (gemModeBuyXp.checked) {
      ...
      currentLevelInput.disabled = false; // Enable current level input
    }
  });
  ...
  gemModeSell.addEventListener('change', () => {
    if (gemModeSell.checked) {
      ...
      currentLevelInput.disabled = true; // Disable current level input when selling
    }
  });
