...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const yourLevelKillInput = document.getElementById('yourLevelKill');
  ...
  xpForm.addEventListener('submit', (ev) => {
    ...
    if (modeTo.checked) {
      ...
      let summaryText = `<div class="gem-highlight"><span class="gem-amount">Level ${target}</span><span class="gem-label">Target Level</span></div>`;
      summaryText += `Total XP required to go from level ${current} to level ${target}: ${formatNumberWithCommas(usedXp)} XP`;
      ...
      // Kill calculator mode
      const yourCurrentLevel = Math.max(1, Math.floor(parseNumberInput(yourLevelKillInput.value) || 30));
      ...
      summaryText += `Eliminating level ${victimCurrentLevel} player (${formatNumberWithCommas(victimTotalXp)} XP)`;
      ...
      if (gemModeSell.checked) {
        ...
        let summaryText = `<div class="gem-highlight"><span class="gem-amount">${formatNumberWithCommas(xpFromGems)} XP</span><span class="gem-label">XP Purchased with Gems</span></div>`;
        ...
        summaryText += `, you reach level ${Math.floor(fractionalLevel)}${progressText}. (Exact: ${fractionalLevel.toFixed(4)})`;
