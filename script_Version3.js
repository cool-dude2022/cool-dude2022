...
(function(){
  ...
  // Track if the user manually edited the target level.
  // If false, target will auto-update to current+1 when current changes.
  ...
  // Initialize defaults: mode = 'to' and target = current + 1
  function initDefaults() {
    // ensure 'to' mode selected
    const toRadio = form.querySelector('input[name="mode"][value="to"]');
    if (toRadio) toRadio.checked = true;

    const current = parseInt(currentLevelInput.value, 10) || 1;
    targetLevelInput.value = Math.max(1, current + 1);
    targetEdited = false;
  ...
  form.addEventListener('change', (e) => {
    if (e.target && e.target.name === 'mode') {
      ...
      if (mode === 'to') {
        toLevelRow.classList.remove('hidden');
        byLevelsRow.classList.add('hidden');
        // if target hasn't been manually edited, keep it in sync with current
        if (!targetEdited) {
          const curr = Math.floor(parseInt(currentLevelInput.value, 10)) || 1;
          targetLevelInput.value = Math.max(1, curr + 1);
        }
      } else {
  ...
  // If the user changes current level and hasn't manually edited target,
  // auto-update target to current+1.
  currentLevelInput.addEventListener('input', () => {
    const curr = Math.floor(parseInt(currentLevelInput.value, 10));
    if (isNaN(curr) || curr < 1) return;
    if (!targetEdited) {
      targetLevelInput.value = Math.max(1, curr + 1);
    }
  });

  // If the user edits the target value, mark it as manually edited to avoid overwrites.
